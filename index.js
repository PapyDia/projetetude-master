...
try{
const email = promptProfEmail(dataObjet.profs);
const professeur = createNewProf(email);

dataObjet.profs.push(professeur);
}catch(err){console.log(err.message);};

function createNewProf (email){
  try{
...
try{
  const cours = createNewCours();

  dataObjet.courses.push(cours);
}catch(err){console.log(err.message);};

// Function créer object cours
...
// Vérifie l'existence déjà du professeur
function promptProfEmail(data) {
  let nouvoEmail = null;
  let count = 0;
  do {
    if (count++ >= 3)
    ...
    // demander de taper l'email de l'etudiant
    nouvoEmail = prompt()("Taper l'email du professeur: ");
    console.log("Nombre de tentatives de mail = ", count);
  } while (data.find((user) => user?.email === nouvoEmail)); //only false if email not found
  // we need false to break the loop
  console.log("retourne mail = ", nouvoEmail);
...
function promptCoursTitre(data) {
  let nouvoTitre = null;
  let count = 0;
  do {
    if (count++ >= 3)
    ...
    // demander de taper l'email de l'etudiant
    nouvoTitre = prompt()("Taper le titre du cours: ");
    console.log("Nombre de tentatives de mail = ", count);
  } while (data.find((user) => user?.titre === nouvoTitre)); //only false if email not found
  // we need false to break the loop
  console.log("retourne mail = ", nouvoTitre);
  return nouvoTitre;
};
